<template>
  <div class="flex items-center h-16 border-b border-gray-300">
    <div class="flex-1 ml-4">
      <input v-model="description" placeholder="Description" class="h-16 bg-transparent border-none outline-none"/>
    </div>
    <div class="mr-2 text-xl font-medium leading-none text-right select-none"
      :class="[ running ? 'text-black' : 'text-gray-200' ]">
      {{ currentTimer }}
    </div>
    <button class="flex justify-center items-center w-16 h-full font-semibold uppercase hover:bg-gray-50 focus:outline-none" @click="toggleTimer">
